...
#     allfilenames = filter(lambda x: '.h5' in x and 'train' in x, allfilenames)
    ...
    nn = 10000
    dataMR = np.zeros([nn,1, dFA[0], dFA[1], dFA[2]], dtype = np.float16)
#     dataMR2D = np.zeros([nn, dFA[0], dFA[1], dFA[2]], dtype = np.float16)
    dataProstate2D = np.zeros([nn,dFA[0], dFA[1], dFA[2]], dtype = np.float16)
#     dataRectum2D = np.zeros([nn,3,168,112], dtype = np.float16)
#     dataBladder2D = np.zeros([nn,3,168,112], dtype = np.float16)
    dataSeg = np.zeros([nn,1,dSeg[0],dSeg[1],dSeg[2]], dtype = np.int8)
#     dataSeg2D = np.zeros([nn,dSeg[0],dSeg[1],dSeg[2]], dtype = np.int8)
    dataContour = np.zeros([nn,1,dSeg[0],dSeg[1],dSeg[2]], dtype = np.int8)
#     dataContour2D = np.zeros([nn,dSeg[0],dSeg[1],dSeg[2]], dtype = np.int8)
#                 
...
#     print allfilenames
    ...
    for i_file, filename in enumerate(allfilenames):
        ...
        with h5py.File(os.path.join(save_dir, filename), 'r+') as h5f:
            ...
            dMR = h5f['dataMR'][:]
#             dMR2D = h5f['dataMR2D'][:]
            dProstate2D = np.squeeze(h5f['dataProstate2D'][:])
...
#             dataBladder2D[startInd : startInd + unitNum,...] = dBladder2D
            dataSeg[startInd : startInd + unitNum,...] = dGT
...
def main():
    ...
    path = '/home/dongnie/warehouse/pelvicSeg/prostateChallenge/pelvicSegRegContourH5/'
    savepath = '/home/dongnie/warehouse/pelvicSeg/prostateChallenge/pelvicSegRegContourBatchH5/'
    
    basePath = '/shenlab/lab_stor5/dongnie/challengeData/'
    path = basePath + 'pelvicSegRegContourH5/'
    savepath = basePath + 'pelvicSegRegContourBatchH5/'
    global savefilename
    savefilename = 'train16x64x64_segregcontour_batch'
