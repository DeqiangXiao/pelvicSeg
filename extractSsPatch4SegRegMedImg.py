...
#                 sitk.WriteImage(volOut,'volRectum_%d.nii.gz'%cubicCnt)
                ...
                trainProstate2D[cubicCnt,:,:,:]=volProstate
    ...
    trainProstate2D=trainProstate2D[0:cubicCnt,:,:,:]
    trainRectum2D=trainRectum2D[0:cubicCnt,:,:,:]
    trainSeg2D=trainSeg2D[0:cubicCnt,:,:,:]

    with h5py.File('./train3x168x112_segreg_%s.h5'%fileID,'w') as f:
        f['dataMR']=trainFA
        f['dataSeg']=trainSeg
    ...
    with open('./trainPelvic3x168x112_all29_segreg_list.txt','a') as f:
